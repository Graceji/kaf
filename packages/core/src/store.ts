...
export const preMiddleware: StoreMiddleware =
  ...
  (action) => {
    ...
    if (!state[moduleName] && action.type !== getInitActionType(moduleName)) {
      return promiseCaseCallback(store.mount(moduleName, "update"), () =>
        next(action)
      );
    }
    return next(action);
...
function applyEffect(
  ...
  decorators.forEach((decorator, index) => {
    decoratorBeforeResults[index] = decorator[0].call(
      model,
      store,
      action,
      effectResult
...
export class Store implements IStore {
  ...
  constructor(
    ...
    const chain = [preMiddleware, ...coreConfig.StoreMiddlewares].map(
  ...
  private respondHandler(
    ...
    const universalActionType = actionName.replace(
      new RegExp(`[^${coreConfig.NSP}]+`),
      "*"
    );
        ...
        orderList.forEach((moduleName) => {
          ...
          effectHandlers.push(
            applyEffect(
              ...
              action,
              this.dispatch,
              handler.__decorators__
            )
          );
        });
        const task =
          effectHandlers.length === 1
            ? effectHandlers[0]
            : Promise.all(effectHandlers);
        return task;
      }
    } else {
      if (isReducer) {
