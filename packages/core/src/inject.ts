...
export function getModule(
  moduleName: string
): Promise<CommonModule> | CommonModule {
...
export function getComponent(
  ...
  const moduleCallback = (module: CommonModule) => {
    const componentOrFun = module.components[componentName] as
      | KAFComponent
      | AsyncKAFComponent;
...
export function getEntryComponent(): KAFComponent {
  return getComponent(
    coreConfig.StageModuleName,
    coreConfig.StageViewName
  ) as KAFComponent;
}
...
export function injectModule(
  moduleName: string,
  moduleGetter: () => CommonModule | Promise<{ default: CommonModule }>
): void;
export function injectModule(
  moduleOrName: string | CommonModule,
  moduleGetter?: () => CommonModule | Promise<{ default: CommonModule }>
): void {
        ...
        actionNames.split(coreConfig.MSP).forEach((actionName) => {
          ...
          if (actionName) {
            actionName = actionName.replace(
              new RegExp(`^this[${coreConfig.NSP}]`),
              `${moduleName}${coreConfig.NSP}`
            );
            ...
            if (arr[1]) {
              ...
              transformAction(
                ...
                handler,
                moduleName,
                handler.__isEffect__
                  ? MetaData.effectsMap
                  : MetaData.reducersMap,
                hmr
              );
            } else {
...
function transformAction(
  ...
  actionHandlerMap: ActionHandlersMap,
  hmr?: boolean
) {
