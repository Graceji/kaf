...
export function nativeUrlToUrl(nativeUrl: string): string {
  ...
  const pathname = coreConfig.NativePathnameMapping.in(
    "/" + path.replace(/^\/|\/$/g, "")
  );
...
export function urlToNativeUrl(kafUrl: string): string {
  ...
  const pathname = coreConfig.NativePathnameMapping.out(
    "/" + path.replace(/^\/|\/$/g, "")
  );
  return `${pathname}${search ? `?${search}` : ""}${hash ? `#${hash}` : ""}`;
}
...
export abstract class BaseNativeRouter {
  ...
  public testExecute(
    ...
    this[testMethod] &&
      this[testMethod](locationToNativeLocation(location), backIndex);
  }
  ...
  public execute(
    ...
    const result: boolean = this[method as string](
      nativeLocation,
      key,
      backIndex
    );
...
export class Router implements IRouter {
  ...
  private onTaskComplete = () => {
    ...
    if (task) {
      ...
      env.setTimeout(
        () => task[0]().finally(onTaskComplete).then(task[1], task[2]),
        0
      );
    } else {
      this.curTask = undefined;
  ...
  addListener(
    callback: (data: RouteEvent) => void | Promise<void>
  ): UNListener {
    this.listenerId++;
  ...
  private addTask(execute: () => Promise<void>): Promise<void> {
    return new Promise((resolve, reject) => {
      const task: RouteTask = [
        () => setLoading(execute(), this.getActivePage().store),
        resolve,
        reject,
      ];
      if (this.curTask) {
  ...
  getHistoryLength(target: RouteTarget): number {
    return target === "window"
      ? this.windowStack.getLength() - 1
      : this.windowStack.getCurrentItem().getLength() - 1;
  }

  getHistory(target: RouteTarget): IRouteRecord[] {
    return target === "window"
      ? this.windowStack.getRecords().slice(1)
      : this.windowStack.getCurrentItem().getItems().slice(1);
  }
  ...
  public init(
    routerInitOptions: RouterInitOptions,
    prevState: StoreState
  ): Promise<void> {
    this.init = () => Promise.resolve();
    this.initOptions = routerInitOptions;
    this.location = urlToLocation(
      nativeUrlToUrl(routerInitOptions.url),
      undefined
    );
    this.action = "init";
  ...
  private async _init() {
    ...
    try {
      ...
      if (
        err.code === ErrorCodes.ROUTE_RETURN ||
        err.code === ErrorCodes.ROUTE_REDIRECT
      ) {
        this.taskList = [];
        throw err;
      }
  ...
  relaunch(
    ...
    return this.addTask(
      this._relaunch.bind(this, partialLocation, target, refresh, _nativeCaller)
    );
  }
  ...
  replace(
    ...
    return this.addTask(
      this._replace.bind(this, partialLocation, target, refresh, _nativeCaller)
    );
  }
  ...
  push(
    ...
    return this.addTask(
      this._push.bind(this, partialLocation, target, refresh, _nativeCaller)
    );
  }
  ...
  private async _back(
    ...
    const { record, overflow, index } = this.windowStack.testBack(
      stepOrKey,
      target === "window"
    );
    if (overflow) {
    ...
    const NotifyNativeRouter =
      (index[0] && coreConfig.NotifyNativeRouter.window) ||
      (index[1] && coreConfig.NotifyNativeRouter.page);
