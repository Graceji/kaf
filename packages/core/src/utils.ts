...
export function promiseCaseCallback<T, R>(
  resultOrPromise: Promise<T> | T,
  callback: (result: T) => Promise<R> | R
): Promise<R> | R {
  if (isPromise(resultOrPromise)) {
...
export function buildConfigSetter<T extends Record<string, any>>(
  data: T
): (config: Partial<T>) => void {
  return (config) =>
